...
class VQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        # if len(answers) > len(samples["question_id"]):
        # qid2ans = {}
        # question_id = samples["question_id"]
        # for answer, ques_id in zip(answers, question_id):
        #     ques_id = int(ques_id.item())
        #     if ques_id in qid2ans:
        #         qid2ans[ques_id].append(answer)
        #     else:
        #         qid2ans[ques_id] = [answer]
        # pred_qa_pairs = []
        # for k, v in qid2ans.items():
        #     if len(v) == 1:
        #         v = v[0]
        #     pred_qa_pairs.append({"question_id": k, "answer": v})
...
@registry.register_task("fvqa")
class FVQATask(VQATask):
    def valid_step(self, model, samples):
        answers = model.predict_answers(
            samples=samples,
            answer_list=self.answer_list,
    ...
    def _report_metrics(self, result_file, split):
        ...
        for res in results:
            ...
            gt_ans = res["gt_ans"].lower()
            pred = res["pred_ans"].lower()

            vqa_acc = 1 if pred in gt_ans or gt_ans in pred else 0
...
class AOKVQATask(VQATask):
    def valid_step(self, model, samples):
        ...
        if 'choices' in samples:
            import string
            if samples["correct_choice_idx"][0] == None:
                gt_answers = samples["correct_choice_idx"]
            else:
                gt_answers = [string.ascii_lowercase[idx] for idx in samples['correct_choice_idx']]

        ans_per_ques = len(question_id) // len(gt_answers)
        new_gt_answers = []
        for b in range(len(gt_answers)):
            new_gt_answers.extend([gt_answers[b]] * ans_per_ques)
